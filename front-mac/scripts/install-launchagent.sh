#!/usr/bin/env bash
# Install / reinstall the LaunchAgent that starts "Mac Disk Monitor.app" at login.
# Usage:
#   ./scripts/install-launchagent.sh           # install + load
#   ./scripts/install-launchagent.sh uninstall # unload + remove
set -euo pipefail

LABEL="com.maximofn.mac-disk-monitor-tray"
SRC="$(cd "$(dirname "$0")" && pwd)/${LABEL}.plist"
DST="$HOME/Library/LaunchAgents/${LABEL}.plist"
APP="$(cd "$(dirname "$0")/.." && pwd)/build/Mac Disk Monitor.app"
...
case "$cmd" in
    install)
        ...
        echo "Logs: ~/Library/Logs/mac-disk-monitor-tray.{out,err}.log"
        ;;
    uninstall)
        if launchctl print "$target" >/dev/null 2>&1; then
            echo "==> bootout $target"
